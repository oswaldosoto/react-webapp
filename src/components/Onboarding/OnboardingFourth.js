...
import onbordingImage from '../../assets/images/onboarding/onboarding04.svg';
import BackIcon from '../../assets/images/arrow.png'

function OnboardingFourth() {
  ...
  return(
    <div className='wrapper'>
      <div id='slide' className='flex flex-wrap text-center max-h-screen max-h-full grid md:grid-cols-2 md:grid-rows-1 '>
        <button onClick={() => navigate('../onboarding/3')} className=' md:hidden sm:block  order-0 float-left top m-0 p-0'>
            <img className='row-span-3 w-20 gap-4 ' alt='BackIcon' src={BackIcon} />
        </button>
        <div className='  items-center '>
            <button onClick={() => navigate('../onboarding/3')} className='hidden md:block order-0 float-left top m-0 p-0'>
                <img className='row-span-3 w-20 gap-4 ' alt='BackIcon' src={BackIcon} />
            </button>
            ...
            <div className='mt-12 place-items-center '>
              <h1 className='text-2xl font-poppins font-extrabold text-tfgreen text-3xl'>Noticias y Precios</h1>
              <h1 className='mt-10'>Recibe las últimas noticias sobre la industria y obtén precios actualizados de tus productos favoritos</h1>
            </div>
        </div>
        ...
        <div className='mt-10 flex items-center justify-center'>
            <button  onClick={() => { navigate('../onboarding/5') } } type="button" className="fixed bottom-0 w-10/12 h-12 text-white font-black mb-5 xl: mt-5 lg:w-1/5 sm:w-full md:w-full bg-tforange  px-6 py-2 border-2 border-orange-400 text-white-200 font-medium text-xs leading-normal uppercase rounded-lg focus:outline-none focus:ring-0 transition duration-150 ease-in-out">Next</button>
        </div>
